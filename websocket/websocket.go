...
type wsHandlerBridge struct {
	...
	batchInBlock *pooledBuffer
	batchInBuf   []byte
	batchTasks   []wsTask
}

var (
	_ fnet.WSFrameAssembler = (*wsHandlerBridge)(nil)
	_ fnet.WSBatchHandler   = (*wsHandlerBridge)(nil)
)
...
func (b *wsHandlerBridge) BeginBatch(estimatedBytes int) {
	b.inBatch = true
	b.batchTasks = b.batchTasks[:0]
	if estimatedBytes > 0 {
		bufSize := 4096
		if estimatedBytes <= 1024+14 {
			bufSize = 1024
		} else if estimatedBytes <= 4096+14 {
			bufSize = 4096
		} else if estimatedBytes <= 16384+14 {
			bufSize = 16 * 1024
		} else {
			bufSize = 64 * 1024
		}
		b.batchInBuf, b.batchInBlock = getPayloadBuffer(bufSize)
		b.batchInBuf = b.batchInBuf[:0]
	} else {
		b.batchInBlock = nil
		b.batchInBuf = nil
	}
...
func (b *wsHandlerBridge) enqueueBatchTask(op OpCode, payload []byte, isCompressed bool) {
	if len(payload) == 0 {
		b.batchTasks = append(b.batchTasks, wsTask{
			op:           op,
			isCompressed: isCompressed,
		})
		return
	}
	if b.batchInBlock == nil {
		bufSize := 4096
		...
		b.batchInBuf, b.batchInBlock = getPayloadBuffer(bufSize)
		b.batchInBuf = b.batchInBuf[:0]
	} else if len(b.batchInBuf)+len(payload) > cap(b.batchInBlock.buf) {
		if len(b.batchTasks) > 0 {
			b.batchTasks[len(b.batchTasks)-1].pooled = b.batchInBlock
		} else {
			putPayloadBuffer(b.batchInBlock)
		}
		bufSize := 16 * 1024
	...
	start := len(b.batchInBuf)
	b.batchInBuf = append(b.batchInBuf, payload...)
	framePayload := b.batchInBuf[start:len(b.batchInBuf)]
...
func (b *wsHandlerBridge) EndBatch() {
	b.inBatch = false
	if b.batchInBlock != nil {
		if len(b.batchTasks) > 0 {
			b.batchTasks[len(b.batchTasks)-1].pooled = b.batchInBlock
	...
	totalBytes := int64(0)
	for i := range b.batchTasks {
		totalBytes += int64(len(b.batchTasks[i].payload))
	}
	...
	if b.closed {
		b.queueMu.Unlock()
		for i := range b.batchTasks {
			if b.batchTasks[i].pooled != nil {
				putPayloadBuffer(b.batchTasks[i].pooled)
			}
		}
		b.batchTasks = b.batchTasks[:0]
		return
	...
	if len(b.batchTasks) > 0 {
		b.queue = append(b.queue, b.batchTasks...)
		b.batchTasks = b.batchTasks[:0]
	}

	if b.maxPendingBytes > 0 && totalBytes > 0 {
		cur := b.pendingBytes.Add(totalBytes)
...
func (b *wsHandlerBridge) processQueue() {
	for {
		...
		for i := range b.batch {
			...
			batchBytes += int64(len(task.payload))
		...
		if b.maxPendingBytes > 0 && batchBytes > 0 {
			cur := b.pendingBytes.Add(-batchBytes)
...
func (b *wsHandlerBridge) OnFrame(h ws.Header, payload []byte) {
	...
	if b.fragOp == 0 {
		...
		if b.inBatch {
			b.enqueueBatchTask(OpCode(h.OpCode), payload, isComp)
		} else {
...
func (c *Conn) writeFrameLocked(op OpCode, payload []byte) error {
	...
	if c.batching {
		...
		if c.batchBlock != nil && len(c.batchBuf)+frameLen > cap(c.batchBlock.buf) {
			if _, err := c.conn.Write(c.batchBuf); err != nil {
				return err
			}
			c.batchBuf = c.batchBuf[:0]
